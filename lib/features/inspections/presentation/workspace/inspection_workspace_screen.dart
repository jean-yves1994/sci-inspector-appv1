...
class _SaveIndicator extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final (text, color) = switch (status) { SaveStatus.idle => ('', Colors.transparent), SaveStatus.saving => ('Saving…', AppColors.textSecondary), SaveStatus.saved => ('Saved', AppColors.success), SaveStatus.failed => ('Not saved', AppColors.danger) };
    if (text.isEmpty) return const SizedBox.shrink();
    return Padding(padding: const EdgeInsets.only(right: AppSpacing.md), child: Center(child: Text(text, style: TextStyle(fontSize: 12, color: color, fontWeight: FontWeight.w600))));
  }
}

class _ProgressBar extends ConsumerWidget {
  const _ProgressBar({required this.inspectionId, required this.onOpenReview});
  final String inspectionId;
  final VoidCallback onOpenReview;
  @override Widget build(BuildContext context, WidgetRef ref) {
    final c = ref.watch(completenessProvider(inspectionId));
    final pct = c.valueOrNull?.percentage ?? 0;
    final outstanding = c.valueOrNull?.outstanding.length ?? 0;
    final refreshing = c.isLoading;
    return SafeArea(child: Padding(padding: const EdgeInsets.all(AppSpacing.md), child: Column(mainAxisSize: MainAxisSize.min, children: <Widget>[
      Row(children: <Widget>[Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(8), child: LinearProgressIndicator(value: refreshing ? null : pct / 100, minHeight: 6, backgroundColor: AppColors.primary.withValues(alpha: .12)))), const SizedBox(width: AppSpacing.sm), Text('$pct%', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 13))]),
      const SizedBox(height: AppSpacing.xs), SizedBox(width: double.infinity, child: FilledButton(onPressed: onOpenReview, child: Text(outstanding == 0 ? 'Review & submit' : 'Review · $outstanding outstanding'))),
    ])));
  }
}

class ReviewPage extends ConsumerStatefulWidget {
  const ReviewPage({required this.inspectionId, required this.onNavigateToSection, super.key});
  final String inspectionId;
...
class _ReviewPageState extends ConsumerState<ReviewPage> {
  bool _busy = false;
  Future<void> _submit(bool resubmit) async {
    final confirmed = await showDialog<bool>(context: context, builder: (c) => AlertDialog(title: Text(resubmit ? 'Resubmit inspection?' : 'Submit inspection?'), content: const Text('I confirm that the information recorded reflects the property inspected and the evidence collected.'), actions: <Widget>[TextButton(onPressed: () => Navigator.of(c).pop(false), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.of(c).pop(true), child: Text(resubmit ? 'Resubmit' : 'Submit'))]));
    if (!(confirmed ?? false)) return;
    setState(() => _busy = true);
    try {
      final updated = await ref.read(inspectionWorkspaceProvider(widget.inspectionId).notifier).submit();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Inspection ${updated.status.label}.')));
      context.pop();
    } on ApiError catch (e) {
      if (!mounted) return;
  ...
  @override Widget build(BuildContext context) {
    final id = widget.inspectionId;
    final completeness = ref.watch(completenessProvider(id));
    final ws = ref.watch(inspectionWorkspaceProvider(id)).valueOrNull;
    final status = ws?.inspection.status ?? InspectionStatus.unknown;
    final canSubmit = status.isEditable;
    return completeness.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (e, _) => ErrorStateView(error: e, onRetry: () => ref.invalidate(completenessProvider(id))),
      data: (c) => ListView(padding: const EdgeInsets.all(AppSpacing.md), children: <Widget>[
        Card(child: Padding(padding: const EdgeInsets.all(AppSpacing.lg), child: Column(children: <Widget>[ProgressGauge(percentage: c.percentage, label: c.complete ? 'Ready' : 'In progress'), const SizedBox(height: AppSpacing.sm), Text(c.complete ? 'Inspection ready for submission' : 'Inspection cannot be submitted yet', textAlign: TextAlign.center, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)), if (!c.complete) Padding(padding: const EdgeInsets.only(top: AppSpacing.xxs), child: Text('${c.outstanding.length} item(s) require attention', style: const TextStyle(fontSize: 12.5, color: AppColors.textSecondary)))]))),
        const SizedBox(height: AppSpacing.md),
        if (c.outstanding.isNotEmpty) ...<Widget>[const SectionLabel('Outstanding requirements'), for (final issue in c.outstanding) Padding(padding: const EdgeInsets.only(bottom: AppSpacing.xs), child: AlertTile(title: issue.message, subtitle: <String?>[issue.sectionCode, issue.fieldCode].where((s) => s != null && s.isNotEmpty).join(' · '), icon: Icons.error_outline_rounded, color: issue.blocking ? AppColors.danger : AppColors.warning, onTap: issue.sectionCode == null ? null : () => widget.onNavigateToSection(issue.sectionCode!)))],
        const SizedBox(height: AppSpacing.md),
        FilledButton(onPressed: canSubmit && !_busy ? () => _submit(status.isCorrection) : null, child: _busy ? const ButtonSpinner() : Text(status.isCorrection ? 'Resubmit inspection' : 'Submit inspection')),
        if (!c.complete) const Padding(padding: EdgeInsets.only(top: AppSpacing.xs), child: Text('The server re-checks completeness at submission. Complete every blocking requirement before submitting.', textAlign: TextAlign.center, style: TextStyle(fontSize: 11.5, color: AppColors.textSecondary))),
