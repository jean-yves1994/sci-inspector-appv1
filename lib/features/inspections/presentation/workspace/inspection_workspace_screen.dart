...
class InspectionWorkspaceScreen extends ConsumerStatefulWidget {
  ...
  /// Deep-link target from a completeness issue (issue.sectionCode).
...
class _InspectionWorkspaceScreenState
  ...
  /// Section codes for the current step list, used for deep-link jumps
  /// without rebuilding the whole step list inside a callback.
  ...
  Widget build(BuildContext context) {
    ...
    return state.when(
      ...
      error: (e, _) => Scaffold(
        ...
        body: ErrorStateView(
          error: e,
          onRetry: () => ref.invalidate(inspectionWorkspaceProvider(id)),
        ),
      ),
      data: (ws) {
        ...
        // Honour the deep link once, after steps exist.
              ...
              child: SizedBox(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, i) => ChoiceChip(
                    ...
                    // Inspectors may move freely between sections.
          ...
          body: Column(
            children: <Widget>[
              if (!enabled)
                MessageBanner(
                  message:
                      'This inspection is ${inspection.status.label.toLowerCase()} '
                      'and is read-only.',
              ...
              if (inspection.status.isCorrection)
                const MessageBanner(
                  message: 'Corrections were requested by the reviewer. '
                      'Update the inspection and resubmit it.',
  ...
  List<_Step> _buildSteps(InspectionTemplate? template, bool enabled) {
    ...
    // 1..n: template-driven field sections (never hard-coded).
    for (final s in template?.fieldSections ?? const <TemplateSection>[]) {
      steps.add(_Step(
        title: s.name,
        code: s.code,
        builder: (_) => TemplateSectionPage(
          inspectionId: id,
          section: s,
          enabled: enabled,
        ),
    ...
    final assessmentSections =
        template?.assessmentSections ?? const <TemplateSection>[];
    ...
    steps.addAll(<_Step>[
      _Step(
        title: 'Owner',
        code: 'OWNER',
        builder: (_) => OwnerPage(inspectionId: id, enabled: enabled),
      ),
      _Step(
        title: 'Valuation',
        code: 'VALUATION',
        builder: (_) => ValuationPage(inspectionId: id, enabled: enabled),
      ),
      _Step(
        title: 'Location',
        code: 'LOCATION',
          ...
          builder: (_) => const Padding(
            ...
            child: MessageBanner(
              message: 'The inspection template could not be loaded. '
                  'Check your connection and refresh — the form is rendered '
                  'from the server template and cannot be shown offline yet.',
            ),
          ),
        ),
      );
...
class _SaveIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (String text, Color color) = switch (status) {
      ...
      SaveStatus.saving => ('Saving…', AppColors.textSecondary),
      SaveStatus.saved => ('Saved', AppColors.success),
      SaveStatus.failed => ('Not saved', AppColors.danger),
    };
...
/// Sticky server-driven progress bar.
...
/// Review & submit. Server completeness is authoritative.
  ...
  Future<void> _submit(bool isResubmit) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        title: Text(isResubmit ? 'Resubmit inspection?' : 'Submit inspection?'),
        content: Text(
          isResubmit
              ? 'Your corrections will be sent back to the reviewer for '
                  'another review.'
              : 'Once submitted, the inspection will be sent to the reviewer '
                  'and most inspection information can no longer be edited.',
        ),
        actions: <Widget>[
          TextButton(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Inspection ${updated.status.label}.')),
      );
      context.pop();
    } on ApiError catch (e) {
      if (!mounted) return;
      // The server re-checks completeness at submit time; trust it over us.
  ...
  Widget build(BuildContext context) {
    ...
    return completeness.when(
      ...
      error: (e, _) => ErrorStateView(
        error: e,
        onRetry: () => ref.invalidate(completenessProvider(id)),
      ),
      data: (c) => ListView(
        padding: const EdgeInsets.all(AppSpacing.md),
        children: <Widget>[
              ...
              child: Column(
                children: <Widget>[
                  ProgressGauge(
                    ...
                    label: c.complete ? 'Ready' : 'In progress',
                  ),
                  const SizedBox(height: AppSpacing.sm),
                  Text(
                    c.complete
                        ? 'Inspection ready for submission'
                        : 'Inspection cannot be submitted yet',
                  ...
                  if (!c.complete)
                    Padding(
                      ...
                      child: Text(
                        '${c.outstanding.length} item(s) require attention',
                        style: const TextStyle(
          ...
          if (c.outstanding.isNotEmpty) ...<Widget>[
            const SectionLabel('Outstanding requirements'),
            for (final issue in c.outstanding)
              Padding(
                padding: const EdgeInsets.only(bottom: AppSpacing.xs),
                child: AlertTile(
                  title: issue.message,
                  subtitle: <String?>[issue.sectionCode, issue.fieldCode]
                  ...
                  onTap: issue.sectionCode == null
                      ? null
                      : () => widget.onNavigateToSection(issue.sectionCode!),
                ),
              ),
          ],
          const SizedBox(height: AppSpacing.md),
